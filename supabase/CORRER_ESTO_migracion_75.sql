-- ============================================================
-- MIGRACION 75 · 28 ago 2026 — REPARACIONES DEL MISMO DIA
--
-- COMO SE CORRE: Ctrl+A, Ctrl+C, y pegar en el editor SQL de Supabase.
-- Correr DESPUES de las 71 a 74.
--
-- POR QUE EXISTE: una revision adversarial encontro que varias migraciones de
-- HOY rompieron cosas que funcionaban. Todo esto repara dano propio.
--
-- LO QUE ARREGLA:
--   1. La migracion 72 borro TRES guardas de la entrega de efectivo, incluida
--      la mas importante: que LA ENTREGA ABRA LA JORNADA. Sin ella, el dinero
--      entregado un dia sin compras no tiene donde constar que se devolvio.
--   2. Dos entregas de EFECTIVO del mismo monto el mismo dia eran imposibles
--      (la regla anti-duplicados comparaba el N de operacion, que en efectivo
--      va vacio).
--   3. El reparto del ajuste del compromiso no podia escribir: la funcion no
--      era security definer. La funcionalidad de las migraciones 65 y 68
--      estaba MUERTA en produccion.
--   4. Pagar con NOTA DE CREDITO estaba bloqueado pidiendo un banco.
--   5. La recepcion perdio el filtro por obra: un almacenero podia recibir
--      material de otra obra.
--   6. stock_fisico() quedo abierta a la clave publica.
--   7. Las cantidades DECIMALES en unidades enteras (2.5 tornillos) entraban:
--      el limite del campo no valida nada sin envio de formulario.
--   8. Aprobar una salida no revalidaba el stock: podia quedar NEGATIVO.
--   9. El boton "Guardar correccion y aprobar" de administracion quedo muerto
--      con la migracion 67.
--
-- AL TERMINAR (los dos primeros deben decir 'true', el ultimo dar 0):
--
--   select
--     (select prosecdef from pg_proc where proname = 'trg_ajuste_al_convertir') as ajuste_definer,
--     (select prosecdef from pg_proc where proname = 'recibir_material')        as recibir_definer,
--     (select count(*) from pg_trigger where tgname = 'zz_cantidad_entera')     as t_enteras,
--     (select count(*) from pg_trigger where tgname = 'zz_salida_revalida_al_aprobar') as t_revalida,
--     (select count(*) from public.rq_items
--       where upper(coalesce(und,'')) in ('UND','PZA','JUEGO','PAR','CAJA','ROLLO','PQT','VARILLA')
--         and cant <> trunc(cant))                                              as cantidades_imposibles;
--
-- OJO: `cantidades_imposibles` probablemente NO de 0 -- son los "2.5 tornillos"
-- que se registraron probando. Se van con el reset; solo hay que saber que
-- estan ahi.
-- ============================================================
