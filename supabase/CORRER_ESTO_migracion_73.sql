-- ============================================================
-- MIGRACION 73 · 28 ago 2026 — los ultimos cabos sueltos
--
-- COMO SE CORRE: Ctrl+A, Ctrl+C, y pegar en el editor SQL de Supabase.
--
-- IMPORTANTE: correr DESPUES de las 71 y 72.
--
-- QUE CIERRA:
--   A · Monica (administracion) puede pagar pero no veia el banco de la obra.
--       BLOQUEANTE: con la migracion 70 corriendo, ademas la deja sin poder
--       pagar. Aparecio comprobando lo demas.
--   B · Un prestamo Solicitado ahora RESERVA el material en el origen: antes
--       el mismo material se podia prometer dos veces y el error reventaba al
--       firmar el segundo residente.
--   C · "El destino ya consumio" contaba las salidas que nadie aprobo todavia,
--       y bloqueaba devoluciones legitimas. Ahora mira el stock FISICO.
--
-- AL TERMINAR (los tres primeros deben dar 1, el ultimo 0):
--
--   select
--     (select count(*) from pg_proc where proname = 'stock_fisico')            as m73_funcion,
--     (select count(*) from pg_policy
--       where polrelid = 'public.proyectos_banco'::regclass
--         and polname = 'proyectos_banco_select')                              as m73_politica,
--     (select count(*) from pg_proc where proname = 'stock')                   as stock_existe,
...
--
-- Si `prometido_dos_veces` sale distinto de 0, hay material comprometido de
-- mas por reservas de antes de esta regla: mandamelo antes de seguir.
-- ============================================================
