-- ============================================================
-- MIGRACION 76 · 28 ago 2026 — UN RQ NACE ENTERO O NO NACE
--
-- COMO SE CORRE: Ctrl+A, Ctrl+C, y pegar en el editor SQL de Supabase.
-- Correr DESPUES de la 75.
--
-- QUE ARREGLA:
--   · El RQ FANTASMA, confirmado con un caso real: dos clics rapidos en
--     "Enviar" dejaron DOS requerimientos, y el primero (RQ-372) nacio urgente
--     y COMPLETAMENTE VACIO -- grabado en la base, visible para Compras como
--     un pedido urgente que no pide nada.
--   · El doble clic: dos envios identicos del mismo residente devuelven el
--     mismo numero en vez de crear otro RQ.
--   · Y BORRA los RQ vacios que ya existen (el RQ-372 entre ellos).
--   · De paso, el mensaje de decimales decia un numero equivocado: el
--     residente escribia 2.5 y leia "no existen 3 und". Era un formato de
--     numero mal escrito en la migracion 75.
--
-- AL TERMINAR (el primero debe dar 1, el segundo 0):
--
--   select
--     (select count(*) from pg_proc where proname = 'crear_rq')          as m76,
--     (select count(*) from public.rqs r
--       where not exists (select 1 from public.rq_items i
--                          where i.rq_id = r.id))                        as rqs_fantasma;
--
-- Y la prueba de verdad, en la aplicacion: dos clics rapidos en Enviar. Tiene
-- que crearse UN solo RQ, con sus materiales.
-- ============================================================
